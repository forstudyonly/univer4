...
public class Folder {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.getCountOfFiles(); i++) {
            hierarchy.append(getStringInternalFile(this.files.get(i), numberOfSpaces));
        }
    ...
    private StringBuilder getStringInternalFolder(Folder folder, int numberOfSpaces) {
        ...
        for (int i = 0; i < folder.getCountOfFiles(); i++) {
            hierarchy.append(getStringInternalFile(folder.files.get(i), numberOfSpaces + 1));
        }
        return hierarchy;
    }

    private String getStringInternalFile(File file, int numberOfSpaces) {
